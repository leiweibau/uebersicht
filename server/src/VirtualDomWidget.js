...
module.exports = function VirtualDomWidget(widgetObject) {
  ...
  function render(state) {
    try {
      contentRoot.render(implementation.render(state, dispatch));
    } catch (err) {
      handleError(err);
    }
  }
  ...
  function renderErrorDetails(details) {
    contentRoot.render(html(ErrorDetails, details));
  }

  api.create = function create() {
    contentEl = document.createElement('div');
    contentRoot = ReactDomClient.createRoot(contentEl);
  ...
  api.destroy = function destroy() {
    commandLoop && commandLoop.stop();
    contentRoot && contentRoot.unmount();
