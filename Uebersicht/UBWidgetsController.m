       ...
       preferences:(UBPreferencesController*)prefs
{
    ...
    if (self) {
        ...
        [header setTitle:NSLocalizedString(@"Widgets", nil)];
        [header setState:0];
...
- (void)render
{
    ...
    for (NSInteger i = widgets.sortedWidgets.count - 1; i >= 0; i--) {
        ...
        if (error) {
            [self notifyUser:error withTitle:NSLocalizedString(@"Error", nil)];
        }
    }
}
...
{
    NSMenuItem* item = [[NSMenuItem alloc]
        initWithTitle: NSLocalizedString(@"Show on main display", nil)
...
{
    NSMenuItem* item = [[NSMenuItem alloc]
        initWithTitle: NSLocalizedString(@"Show on all screens", nil)
...
{
    NSMenuItem* item = [[NSMenuItem alloc]
        initWithTitle: NSLocalizedString(@"Hide widget", nil)
                             ...
                             forWidget:(NSString*)widgetId
{
    ...
    [item setTitle:NSLocalizedString(@"Show on selected screens:", nil)];
...
{
    NSMenuItem* item = [[NSMenuItem alloc]
        initWithTitle: NSLocalizedString(@"Send to background", nil)
...
- (void)editWidget:(id)sender
{
    ...
    if (![[NSWorkspace sharedWorkspace] openFile:filePath]) {
        NSString* message = NSLocalizedString(
            @"Please configure an app to edit .%@ files",
            nil
        );
        [self
            notifyUser: [NSString
            ...
            withTitle: NSLocalizedString(@"No Editor Configured.", nil)
        ];
    }
}
