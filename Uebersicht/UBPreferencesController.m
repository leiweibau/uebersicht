...
- (void)localizePreferencesUI
{
    self.window.title = NSLocalizedString(self.window.title, nil);
    [self localizeViewTree:self.window.contentView];
}

- (void)localizeViewTree:(NSView*)view
{
    if ([view isKindOfClass:[NSButton class]]) {
        NSButton* button = (NSButton*)view;
        if (button.title.length > 0) {
            button.title = NSLocalizedString(button.title, nil);
        }
        if (button.alternateTitle.length > 0) {
            button.alternateTitle = NSLocalizedString(button.alternateTitle, nil);
        }
    } else if ([view isKindOfClass:[NSTextField class]]) {
        NSTextField* textField = (NSTextField*)view;
        if (!textField.editable && !textField.selectable) {
            NSString* value = textField.stringValue;
            if (value.length > 0) {
                textField.stringValue = NSLocalizedString(value, nil);
            }
        }
    }

    for (NSView* subview in view.subviews) {
        [self localizeViewTree:subview];
    }
}
