...
If you like to make Ajax requests to an external site without using a command, you can use the built in proxy server. It runs on `http://127.0.0.1:41417` and can be used as follows:
...
Security notes:
  ...
  - Only requests from the local Übersicht app origin are accepted.
  - Only `GET` and `HEAD` are allowed.
  - Requests to loopback/private-address hosts are blocked by default.
  - Set `UEBERSICHT_PROXY_ALLOWLIST` to a comma-separated host allowlist (supports `*\.example.com`) for stricter control.
